...
def update_stock_on_order_item_create(sender, instance, created, **kwargs):
    """
    Update product stock when an OrderItem is created or updated.
...
def restore_stock_on_order_item_delete(sender, instance, **kwargs):
    """
    Restore product stock when an OrderItem is deleted.
    """
    try:
        with transaction.atomic():
            product = Product.objects.select_for_update().get(id=instance.product.id)
            ...
            for item in instance.items.all():
                try:
                    with transaction.atomic():
                        ...
                        print(f"🔄 Order {instance.id} cancelled: Restored {item.quantity} units of {product.name}")
                except Product.DoesNotExist:
                    continue
            ...
            for item in instance.items.all():
                try:
                    with transaction.atomic():
                        ...
                        print(f"🔄 Order {instance.id} un-cancelled: Reserved {item.quantity} units of {product.name}")
                except (Product.DoesNotExist, Exception) as e:
                    print(f"⚠️ Could not reserve stock for {item.product.name}: {e}")
                    continue
...
def validate_stock_before_order(order_items):
    ...
    Args:
        order_items: List of dictionaries with 'product' and 'quantity' keys
...
def create_order_with_stock_validation(user, order_items_data, **order_kwargs):
    ...
    Args:
        ...
        order_items_data: List of dicts with 'product', 'quantity', 'price' keys
